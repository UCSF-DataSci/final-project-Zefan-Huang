"""
Stage 15.4 true external-case end-to-end inference.

This entrypoint accepts raw external case inputs, builds a single-case Stage 9
pack, reruns deterministic Stage 10/11 feature construction, applies the best
available Stage 13 checkpoint, and exports the same Stage 15 deliverables used
for cohort cases.
"""
import argparse
import csv
...
<head>
  <meta charset="utf-8"/>
  <title>External Inference Bundle: {patient_id}</title>
  <style>
    body {{
      margin: 0;
      padding: 32px;
...
<body>
  <div class="panel">
    <h1>External Inference Bundle: {patient_id}</h1>
